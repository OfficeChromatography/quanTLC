...
if("Integration options" %in% input$Report_options && !is.null(reac$Integration_start)){
    ...
    Integration_start = round(seq((width-dist.bas)/(Zf-dist.bas),-dist.bas/(Zf-dist.bas),length.out=dim(reac$preprocessed)[2])[mean(reac$Integration_start)],2)
    Integration_stop = round(seq((width-dist.bas)/(Zf-dist.bas),-dist.bas/(Zf-dist.bas),length.out=dim(reac$preprocessed)[2])[mean(reac$Integration_stop)],2)
