...
f.plot.array<-function(data,id,hauteur,Zf,dist.bas,reconstruct=T,inverse=F,ylim.raster=1.3,main=NULL,xlim=NULL,...){
  ...
  if(reconstruct==T){
    plot(as.vector(data[id,,1]),
         ylim=c(0,ylim.raster),xlim=xlim,main=main,xlab=expression(italic(hR)['F']),ylab='Pixel intensity [AU]',xaxt="n",
         type="l",col="red",...)
    par(new=T)
    ...
    plot(as.vector(data[id,,1]),
         ylim=c(min(data),max(data)),xlim=xlim,main=main,xlab=expression(italic(hR)['F']),ylab='Pixel intensity [AU]',xaxt="n",
         type="l",col="red",...)
    par(new=T)
    plot(as.vector(data[id,,2]),
